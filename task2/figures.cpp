...
Triangle::Triangle(double a, double b, double c) : a(a), b(b), c(c) {
    if (a <= 0 || b <= 0 || c <= 0) {
        throw LessThanZeroParam(); // Ловим отрицательные значения параметров
    }
    // Проверка на существование треугольника
    if (a + b <= c || a + c <= b || b + c <= a) {
        throw WrongTriangle(); // Ловим некорректные треугольники
    }
}
...
Circle::Circle(double radius) : radius(radius) {
    if (radius <= 0) {
        throw LessThanZeroParam(); // Ловим отрицательное значение радиуса
    }
}
...
// make_figure function implementation
